...
// TODO: Provide public method to indirectly access private method
//
//go:linkname searchSeriesWithTagArray github.com/openGemini/openGemini/engine/index/tsi.(*MergeSetIndex).searchSeriesWithTagArray
func searchSeriesWithTagArray(idx *tsi.MergeSetIndex, tsid uint64, seriesKeys [][]byte, exprs []*influxql.BinaryExpr, combineKey []byte,
...
func (e *Exporter) writeSeriesRecords(outputWriter io.Writer, sid uint64, rec *record.Record, index *tsi.MergeSetIndex) error {
	...
	if seriesKeys, _, isExpectSeries, err = searchSeriesWithTagArray(index, sid, seriesKeys, nil, combineKey, isExpectSeries, nil, false); err != nil {
		return err
	}
	series := make([][]byte, 1)
	sIndex := 0
...
func (e *Exporter) writeSingleWalFile(file string, outputWriter io.Writer) error {
	...
	recordCompBuff := bufferpool.NewByteBufferPool(engine.WalCompBufSize, cpu.GetCpuNum(), bufferpool.MaxLocalCacheLen).Get()
	var offset int64 = 0
	var rows []influx.Row
	for {
